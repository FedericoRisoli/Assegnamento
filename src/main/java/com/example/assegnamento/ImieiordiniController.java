...
public class ImieiordiniController {
    ...
    private void initialize() throws SQLException, IOException {
        ...
        r = DBHelper.query("SELECT `nome`,`cognome`,`ordine`,`indirizzo`,`dataconsegna` FROM `ordinivendita` WHERE `nome` LIKE \""+data.Getname()+"\" AND `cognome` LIKE \""+data.Getsurname()+"\"  ORDER BY `dataconsegna`;");
